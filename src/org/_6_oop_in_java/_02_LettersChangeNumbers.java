...
//Nakov likes Math. But he also likes the English alphabet a lot. He invented a game with numbers and letters from the English
// alphabet. The game was simple. You get a string consisting of a number between two letters.
...
// If it’s lowercase you add its position to the resulted number. But the game became too easy for Nakov really quick.
// He decided to complicate it a bit by doing the same but with multiple strings keeping track of only the
// total sum of all results. Once he started to solve this with more strings and bigger numbers it became quite hard to do it
// only in his mind. So he kindly asks you to write a program that calculates the sum of all numbers after the operations on each
// number have been done.
...
public class _02_LettersChangeNumbers {
    ...
    public static void main(String[] args) {
        ...
        for (String word : tokens) {
            ...
            if(Character.isUpperCase(firstCharacter)){
                number /= GetPositionInAlphabet(firstCharacter);
            }
            else {
                number *= GetPositionInAlphabet(firstCharacter);
            }
            if (Character.isUpperCase(lastCharacter)){
                number -= GetPositionInAlphabet(lastCharacter);
            }
            else {
                number += GetPositionInAlphabet(lastCharacter);
            }
    ...
    private static int GetPositionInAlphabet(Character ch) {
        char[] lowerCaseCharacter = ch.toString().toLowerCase().toCharArray();

        return ((int) lowerCaseCharacter[0] - (int) 'a') + 1;
    }
}
